  ...
  */
  ...
  function(absoluteFilepath, expects, callback) {
    ...
    fs.stat(absoluteFilepath, function(err, stats) {
      if (err) { return callback(new errors.cache.FsError(err)); }
      mtime = stats.mtime.getTime();
      /**
      * For file-system cache, we do not use the expects param
      * we read from the cache immediately
      */
      if (! expects) {
        var key = composeKey(absoluteFilepath);
        _this._cache.get(key, function(err, val) {
          if (err) {
            _this.debug("errored getting data using only" +
              " filepath");
      ...
      _this._cache.get(key, function(err, data) {
        ...
        if (data) {
          ...
          if (! item.isValid(mtime)) {
            /**
            * Since the item is invalid due to the original backing file
            * being modified, we shall unset all items from cache
            * that may have content from the file at the
            * absoluteFilepath original or converted
            */
            _this._cache.unset(key);
            for (var i = index; i < expects.length; i++) {
              key = composeKey(absoluteFilepath, expects[i]);
              _this._cache.unset(key);
            }
            // data in cache is not valid, we stop!!!
            return callback(null, null, null);
          }
