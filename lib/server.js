...
app.get("/files/", function(req, res) {
  ...
  var options = _.merge(req.query, config);
...
app.get("/file/", function(req, res) {
  debug("reading file");
  var options = _.merge(req.query, config);
  var datatype = mime.lookup(req.query.path);
  var expects = req.query.expects || [ "text/html" ];
  fbr.handle(options, function(err, descriptor) {
    if (err) {
      return res.json({ error: err });
    }
...
app.get("/plugins/www", express.static(utils.getPath("app.plugins")));
...
function startServer(options, callback) {
  if (!callback) {
    if (_.isFunction(options)) {
      ...
      callback = utils.defineCallback();
    }
  }
  options = _.merge({}, config.server, options);
