 ...
 *
 * @example
 * getAccepts("text/html,application/json"); // => ["text/html", "application/json"]
 *
...
function getAccepts(acceptString) {
  var accepts = acceptString.split(/\,/g);
  accepts.forEach(function(accept, index) {
    accepts[index] = accept.replace(/\ |\n|\t/g, "");
  });
  return accepts;
}
