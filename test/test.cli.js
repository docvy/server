...
describe("CLI", function() {
  ...
  it("-s starts server in daemon mode", function(done) {
    shell.exec(cmd("-s"), function(code, output) {
      code.should.eql(0);
      setTimeout(function() {
  ...
  it("-x stops server", function(done) {
    server.start(function() {
      shell.exec(cmd("-x"), function(code, output) {
        code.should.eql(0);
        request.get(url, function(err, res, body) {
          err.should.be.ok;
  ...
  it("-p allows passing port number", function(done) {
    var _port = 9821;
    shell.exec(cmd("-s -p " + _port), function(code, output) {
      request.get("http://localhost:" + _port, function(err, res, body) {
        should(err).not.be.ok;
        request.del(url + "/stop", function() { done(); });
      }); // request.get
    }); // shell.exec
